...
if training_type == 'observational':
    train = pd.read_csv('cp_training_obs.csv', sep=';')
elif training_type == 'pooled':
  train = pd.read_csv('cp_training_pooled.csv', sep = ';')
test = pd.read_csv('cp_test.csv', sep=';')
...
out = utils.fit_pred_cp(train, test, rho, alpha, this_seed)
out['env'] = e
out['training'] = training_type

## export results
out.to_csv(
    'results_' + e + '_' + str(rho) + '_' + training_type + '.csv',
    index = False,
    sep = ';'
)
